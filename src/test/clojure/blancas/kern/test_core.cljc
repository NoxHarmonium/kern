...
(deftest test-0005
  (let [em "the buck stops here"
    ...
    (testing "fail"
      ...
      (is (= false (:ok    s2)))
      (is (= true (:empty s2)))
      (is (= (make-err-message (:pos s1) em) (:error s2)))
      (is (= (:input s1) (:input s2)))
      (is (= (:pos   s1) (:pos   s2)))
      (is (= (:user  s1) (:user  s2)))))))
...
(deftest test-0010
  (let [s1 (parse letter "xyz")
    ...
    (testing "satisfy - advances one char"
      ...
      (is (= (:user s1) (:user  s2)))
      (is (= nil (:error s2)))
      (is (= {:line 1 :col 3} (select-keys (:pos   s2) [:line :col]))))))
...
(deftest test-0020
  (let [s1 (parse letter "u\t")
    ...
    (testing "satisfy - advnaces one tab; default 4 positions"
      ...
      (is (= (:user s1) (:user  s2)))
      (is (= nil (:error s2)))
      (is (= {:line 1 :col 6} (select-keys (:pos   s2) [:line :col]))))))
...
(deftest test-0025
  (binding [*tab-width* 8]
    (let [s1 (parse letter "u\t")
      ...
      (is (= (:user s1) (:user  s2)))
      (is (= nil (:error s2)))
      (is (= {:line 1 :col 10} (select-keys (:pos   s2) [:line :col]))))))))
...
(deftest test-0030
  (let [s1 (parse letter "u\n")
    ...
    (testing "satisfy - advances to the next line, first column"
      ...
      (is (= (:user s1) (:user  s2)))
      (is (= nil (:error s2)))
      (is (= {:line 2 :col 1} (select-keys (:pos   s2) [:line :col]))))))
...
(deftest test-0035
  (let [em "end of input"
    ...
    (testing "satisfy - attempts to read past the end of input"
      ...
      (is (= true (:empty s2)))
      (is (= (:user s1) (:user  s2)))
      (is (= (make-err-unexpect (:pos s1) em) (:error s2)))
      (is (= (:pos s1) (:pos   s2)))))))
...
(deftest test-0040
  (let [em "\\2"
	s1 (parse (satisfy #(Character/isLetter %)) "u2")
    ...
    (testing "satisfy - the predicate fails"
      ...
      (is (= true (:empty s2)))
      (is (= (:user s1) (:user  s2)))
      (is (= (make-err-system (:pos s1) em) (:error s2)))
      (is (= (:pos s1) (:pos   s2)))))))
...
(deftest test-0045
  (let [in "#(f %)"
    ...
    (testing "any-char"
      ...
      (is (= (:user s1) (:user  s1)))
      (is (= nil (:error s1)))
      (is (= {:line 1 :col 2} (select-keys (:pos   s1) [:line :col]))))))
...
(deftest test-0055
  (let [em "end of input"
    ...
    (testing "any-char - fails on end of input"
      ...
      (is (= true (:empty s1)))
      (is (= (:user s1) (:user  s1)))
      (is (= (make-err-unexpect (:pos s1) em) (:error s1)))
      (is (= {:line 1 :col 1} (select-keys (:pos   s1) [:line :col])))))))
...
(deftest test-0075
  (let [in "abc"
	s1 (parse lower in)]
    (testing "lower - parses a single lower-case letter"
      (is (= (rest in) (:input s1)))
      (is (= \a (:value s1)))
      (is (= true (:ok    s1)))
      (is (= false (:empty s1))))))
		       ...
		       (fn [a]
				...
				(>>= lower
				     (fn [c]
				       (return [a b c])))))))
		  in)]
    (testing "lower - three in a row until end of input"
      (is (empty? (:input s1)))
      (is (= (seq in) (:value s1))))))
...
(deftest test-0090
  (let [in "ABC"
	s1 (parse upper in)]
    (testing "upper - parses a single upper-case letter"
      (is (= (rest in) (:input s1)))
      (is (= \A (:value s1)))
      (is (= true (:ok    s1)))
      (is (= false (:empty s1))))))
		       ...
		       (fn [a]
				...
				(>>= upper
				     (fn [c]
				       (return [a b c])))))))
		  in)]
    (testing "upper - three in a row until end of input"
      (is (empty? (:input s1)))
      (is (= (seq in) (:value s1))))))
...
(deftest test-0105
  (let [in " \t\t"
    ...
    (testing "white-space - parses a single whitespace character"
      (is (= (rest in) (:input s1)))
      (is (= \space (:value s1)))
      (is (= true (:ok    s1)))
      (is (= false (:empty s1))))))
		       ...
		       (fn [a]
				...
				(>>= white-space
				     (fn [c]
				       (return [a b c])))))))
		  in)]
    (testing "white-space - three in a row until end of input"
      (is (empty? (:input s1)))
      (is (= (seq in) (:value s1))))))
...
(deftest test-0250-65
  (let [in "subroutine"
    ...
    (testing "word* - fails with incorrect input"
      ...
      (is (= false (:ok    s1)))
      (is (= true (:empty s1)))
      (assert-error "subr\nunexpected subrou" "proc, func or method" em))))
...
(deftest test-0280-30
  (let [in "12345 "
    ...
    (testing "new-line* - the line doesn't end with a new line"
      ...
      (is (= false (:ok    s1)))
      (is (= false (:empty s1)))
      (assert-error space-string "new line" em))))
...
(deftest test-0305
  (let [s1 (parse (<?> (<*> digit letter) "digit,letter") "0")
    	em (get-msg-str (:error s1))]
    (testing "<?> - verifies error messages in <*>"
      (assert-error "end of input" "letter" em))))
...
(deftest test-0310
  (let [s1 (parse (<|> digit letter) "*")
    ...
    (testing "<|> - verifies error messages"
      (assert-error "*" "digit or letter" em))))
...
(deftest test-0315
  (let [s1 (parse (<|> (sym* \x) (<|> letter digit)) "*")
    ...
    (testing "<|> - verifies error messages with 3 choices"
      (assert-error "*" #?(:clj "x, letter or digit" :cljs "\"x\", letter or digit") em))))
...
(deftest test-0320
  (let [s1 (parse (<|> (<|> white-space (sym* \x)) (<|> letter digit)) "*")
    ...
    (testing "<|> - verifies error messages with 4 choices"
      (assert-error "*" #?(:clj "whitespace, \\x, letter or digit" :cljs "whitespace, \"x\", letter or digit") em))))
...
(deftest test-0340
  (let [s1 (parse (<|> white-space letter digit) "*")
	em (get-msg-str (:error s1))]
    (testing "<|> - verifies error messages with 3 choices"
      (assert-error "*" "whitespace, letter or digit" em))))
...
(deftest test-0370
  (let [p1 (>>= letter (fn [x] (return #?(:clj (Character/toUpperCase x) :cljs (.toUpperCase (str x))))))
	s1 (parse p1 "xyz")]
...
(deftest test-0375
  (let [p1 (>>= digit
		(fn [x] (>>= digit
			     (fn [y] (return #?(:clj (Integer/parseInt (str x y)) :cljs (js/parseInt (str x y) 10)))))))
	s1 (parse p1 "50113")]
    (testing ">>= - advances two chars"
      ...
      (is (= nil (:user  s1)))
      (is (= nil (:error s1)))
      (is (= {:line 1 :col 3} (select-keys (:pos   s1) [:line :col]))))))
...
(deftest test-0385-05
  (let [p1 (bind [x letter] (return #?(:clj (Character/toUpperCase x) :cljs (.toUpperCase (str x)))))
	s1 (parse p1 "xyz")]
...
(deftest test-0385-10
  (let [p1 (bind [x digit y digit]
	     (return #?(:clj (Integer/parseInt (str x y)) :cljs (js/parseInt (str x y) 10))))
	s1 (parse p1 "50113")]
...
(deftest test-0385-15
  (let [p1 (bind [_ (sym* \()
		  s (<+> (many1 digit))
		  _ (sym* \))]
	     (return (* #?(:clj (Integer/parseInt s) :cljs (js/parseInt s 10)) -1)))
	s1 (parse p1 "(50113)")]
...
(deftest test-0385-35
  (let [p1 (bind [x letter] #?(:clj (Character/toUpperCase x) :cljs (.toUpperCase (str x))))
	s1 (parse p1 "xyz")]
    (testing "bind with autolift - advances one char"
      (is (= [\y \z] (:input s1)))
      (is (= \X (:value s1)))
      (is (= true (:ok    s1)))
      (is (= false (:empty s1)))
...
(deftest test-0385-40
  (let [p1 (bind [x digit y digit]
	     #?(:clj (Integer/parseInt (str x y)) :cljs (js/parseInt (str x y) 10)))
	s1 (parse p1 "50113")]
    (testing "bind with autolift - advances two chars"
      (is (= [\1 \1 \3] (:input s1)))
      (is (= 50 (:value s1)))
      (is (= true (:ok    s1)))
      (is (= false (:empty s1)))
...
(deftest test-0385-45
  (let [p1 (bind [_ (sym* \()
		  s (<+> (many1 digit))
		  _ (sym* \))]
	     (* #?(:clj (Integer/parseInt s) :cljs (js/parseInt s 10)) -1))
	s1 (parse p1 "(50113)")]
    (testing "bind with autolift - reads a negative number in parens, as in accounting"
      (is (empty? (:input s1)))
      (is (= -50113 (:value s1)))
      (is (= true (:ok    s1)))
      (is (= false (:empty s1)))
...
(deftest test-0385-50
  (let [p1 (bind [x letter]
	...
	s1 (parse p1 "xYZ")]
    (testing "bind with autolift - uses nested bind inside the first function body"
      (is (empty? (:input s1)))
      (is (= "first" (:value s1)))
      (is (= true (:ok    s1)))
      (is (= false (:empty s1)))
...
(deftest test-0385-60
  (let [p1 (<|> (bind [x (many1 digit)] true)
	...
	s1 (parse p1 "FALSE")]
    (testing "bind with autolift - the first bind fails, the second succeeds"
      (is (empty? (:input s1)))
      (is (= false (:value s1)))
      (is (= true (:ok    s1)))
      (is (= false (:empty s1)))
...
(deftest test-0530
  (let [s1 (parse (<:> (<*> upper lower upper)) "Mi*")
    ...
    (testing "<:> - fails with parsers consuming input; consumes no input"
      ...
      (is (= false (:ok    s1)))
      (is (= true (:empty s1)))
      (assert-error "*" "uppercase letter" em))))
...
(deftest test-0540
  (let [s1 (parse (<|> (<:> letter) digit) "***")
    ...
    (testing "<:> - verifies that it carries over the error msg"
      ...
      (is (= false (:ok    s1)))
      (is (= true (:empty s1)))
      (assert-error "*" "letter or digit" em))))
...
(deftest test-0575
  (let [s1 (parse (many1 lower) "*")
    ...
    (testing "many1 - fails with zero items"
      ...
      (is (= false (:ok    s1)))
      (is (= true (:empty s1)))
      (assert-error "*" "lowercase letter" em))))
...
(deftest test-0595
  (let [s1 (parse (<|> (many1 digit) upper) "*")
    ...
    (testing "many1 - fails; passes on empty, errors to <|>"
      ...
      (is (= false (:ok    s1)))
      (is (= true (:empty s1)))
      (assert-error "*" "digit or uppercase letter" em))))
...
(deftest test-1045
  (let [s1 (parse (not-followed-by (<*> upper digit)) "U2*")
    ...
    (testing "not-followed-by - fails as the parse succeeds"
      ...
      (is (= false (:ok    s1)))
      (is (= true (:empty s1)))
      (is (= #?(:clj "unexpected [\\U \\2]" :cljs "unexpected [\"U\" \"2\"]") em)))))
...
(deftest test-1300
  (let [p1 (>> (put-state 0)
    ...
    (testing "Parse from a text file: put-state, modify-state"
      (is (empty? (:input s1)))
      (is (= nil (:value s1)))
      (is (= true (:ok    s1)))
      (is (= 12 (:user  s1))) ; text file has a trailing newline.
      (is (= false (:empty s1)))))))
...
(deftest test-1310
  (let [file "src/test/resources/test-1310.txt"
	...
	em (get-msg-str (:error s1))]
    (testing "Parse from a text file -- get an error"
      (is (= (seq in) (:input s1)))
      (is (= nil (:value s1)))
      (is (= false (:ok    s1)))
      (is (= true (:empty s1)))
...
(deftest test-1320
  (let [file "src/test/resources/test-1310.txt"
    ...
    (testing "Parse from a file and call print-error"
      (is (= (str "src/test/resources/test-1310.txt line 1 column 6\n"
                  "unexpected e\n"
...
(deftest test-1330
  (let [file (java.io.FileReader. "src/test/resources/test-1310.txt")
	s1 (parse-file (word* letter "proc") file)
	em (get-msg-str (:error s1))]
    (testing "Read from a file reader"
      (is (= true (:empty s1)))
      (assert-error "e" "end of proc" em)))))
...
(deftest test-1340
  (let [file (java.io.FileReader. "src/test/resources/test-1310.txt")
	s1 (parse-file (word* letter "proc") file)
        pe (with-out-str (print-error s1))]
    (testing "Read from a file reader and call print-error"
      (is (= (str "line 1 column 6\n"
                  "unexpected e\n"
                  "expecting end of proc\n") pe))))))
...
(deftest test-1350
  (let [file (java.io.File. "src/test/resources/test-1310.txt")
	s1 (parse-file (word* letter "proc") file)
        pe (with-out-str (print-error s1))]
    (testing "Read from a java.io.File instance and call print-error"
      (is (= (str "line 1 column 6\n"
                  "unexpected e\n"
                  "expecting end of proc\n") pe))))))
